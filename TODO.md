...
- Priority: `P1`
- Status: `Done`
- Files: `internal/report/report.go`, `internal/report/golden_test.go`, `cmd/compatibility-lab/main.go`, `testdata/goldens/compatibility-report.md`
...
- Notes: Added `report.WriteMarkdown` that renders a PR-comment-ready document: an H1 title, a schema/inputs header, a summary counts table (Ready / Warning / Unknown / Blocked / Provider / MRMO), and then sections in the same `blocked > warning > unknown > ready` precedence order the matrix package defines — so a reviewer's eye always lands on actionable findings first. Blocked, Warning, and Unknown resources render as verbose blocks with an `### `<terraformType>`` heading and bullet lists (MRMO ref, tier, score, issue codes with `**severity · `code`** — message` formatting) because the reader needs the "why". The Ready bucket collapses into a `<details><summary>Ready (N)</summary>` block containing a compact table (Terraform Type / MRMO Ref / Tier / Score) because there can be a lot of them and they don't need inline attention. The writer never re-sorts input — it consumes the alphabetized `Resources` slice from LAB-3 as-is, so the markdown inherits LAB-3's determinism guarantee for free. Wired `--format markdown` into `runScan` and the usage banner. Locked with `TestMarkdownReportGolden` in `internal/report/golden_test.go`: it scans the LAB-5 fixture provider + MRMO trees, freezes `RepoPath` to `PROVIDER_FIXTURE`/`MRMO_FIXTURE` so the golden stays portable, and compares the byte-exact markdown output to `testdata/goldens/compatibility-report.md` via `testutil.AssertTextGolden` (regenerate with `go test ./... -update`). Verified end-to-end against the fixture tree: `scan --format markdown` produces a 48-line document that renders cleanly on GitHub with all four status sections populated, and empty sections are correctly elided (the fixture has zero Ready resources so no Ready block appears).
...
- Priority: `P2`
- Status: `Done`
- Files: `internal/providerdiff/providerdiff.go`, `internal/providerdiff/git.go`, `internal/providerdiff/providerdiff_test.go`, `internal/report/report.go`, `internal/report/golden_test.go`, `cmd/compatibility-lab/main.go`, `testdata/goldens/provider-diff-report.md`
...
- Notes: Built a new package `internal/providerdiff` that owns the analytical core. `providerdiff.Diff(base, head model.ProviderManifest, mrmoManifest model.MRMOManifest) DiffReport` is a pure function that walks both manifests, keys resources by `TerraformType`, and emits a `Finding` per detected change with one of twelve `Kind` values (`RESOURCE_ADDED`/`REMOVED`, `EXPORTER_ADDED`/`REMOVED`, `REFATTR_ADDED`/`REMOVED`/`CHANGED`, `ENCODED_REFATTR_ADDED`/`REMOVED`/`CHANGED`, `SINGLETON_FLIPPED`, `EXPORT_ID_CHANGED`). The `Risk` vocabulary (`high` / `medium` / `low`) is driven by LAB-7's acceptance criteria: removed exporters, removed/changed `RefAttrs`, removed/changed `EncodedRefAttrs`, and removed resources escalate to `high` when the Terraform type appears in `mrmoManifest`; `SINGLETON_FLIPPED` and `EXPORT_ID_CHANGED` are always `medium`; every `_ADDED` is `low`. Findings are sorted deterministically by (TerraformType, Kind, Attribute, BeforeValue) and a nil slice is rewritten to `[]` so `findings: null` never appears in JSON. Owns its own contract at `SchemaVersion = "compatibility-lab-diff/v1"` — separate from `matrix.SchemaVersion` because a diff and a compatibility scan are different documents. Git integration lives in `internal/providerdiff/git.go`: `CompareRefs(repo, base, head)` uses `git worktree add --detach` into two `os.MkdirTemp` directories, runs `provider.Scan` on each, and returns a `cleanup` closure that shells `git worktree remove --force` plus `os.RemoveAll` even on error so `.git/worktrees/` never leaks. CLI: `diff-provider-pr` supports two operational modes — git-ref (`--provider-repo` + `--base` + `--head`) and manifest-file (`--base-manifest` + `--head-manifest`, for CI pipelines that already persist scans as JSON artifacts) — plus `--mrmo-repo` for risk grading, `--format table|json|markdown`, and `--strict` (exits non-zero when any `high` finding exists, mirroring `scan --strict`). Table output is width-stable and grouped by risk descending; markdown output mirrors the LAB-4 layout (title / schema+refs header / summary table / severity-ordered sections with backtick-formatted transitions like `` `genesyscloud_flow` → `genesyscloud_new_flow_type` ``) so both `scan` and `diff-provider-pr` present findings in the same visual language. Tests: 8 unit tests in `providerdiff_test.go` cover every Kind and both risk paths — the direct acceptance test `TestDiff_ExporterRemovedOnMRMOSupportedIsHighRisk`, the mirror `TestDiff_ExporterRemovedOnUnmanagedIsLowRisk`, `TestDiff_RefAttrRemovedRiskGrading` (both directions), `TestDiff_RefAttrAddedAndChanged`, `TestDiff_EncodedRefAttrsAndSingletonAndExportID`, `TestDiff_ResourceAddedAndRemoved`, `TestDiff_IsDeterministic` (6× repeat), and `TestDiff_EmptyReturnsEmptyFindingsNotNil` (JSON contract). Plus `TestDiffReportMarkdownGolden` in `internal/report/golden_test.go` hand-mutates the LAB-5 fixture provider (removes `auth_division` entirely, drops `routing_queue`'s exporter, renames a `RefAttr` and adds one, changes `architect_flow`'s `ExportID`, adds a brand-new resource) and asserts the markdown output byte-matches `testdata/goldens/provider-diff-report.md`. Verified end-to-end: `--strict` returns exit code 1 when high-risk findings exist, 0 when clean; live git-worktree run against `~/genesys_src/terraform-provider-genesyscloud` (comparing initial commit to HEAD) completes cleanly and `git worktree list` post-run confirms no orphaned entries under `.git/worktrees/`.
